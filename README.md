...
images:                              # 선택, 최대 3개. 첫 번째가 목록 카드 썸네일
  - /static/images/cover.png         # 없으면 목록에 아이콘 플레이스홀더 표시
  - /static/images/screen-2.png
---
...
> 이미지는 최대 3개까지 등록할 수 있습니다. 목록 카드에는 **첫 번째 이미지**가 썸네일로
> (고정 비율로 잘려서) 표시되고, 상세보기에서는 **모든 이미지가 잘리지 않고 원본 비율 그대로**
> 세로로 나열됩니다. (레거시 `thumbnail:` 단일 필드도 계속 동작합니다.)
...
---
company: 브라이트벨
position: 개발부서 / 사원   # 부서·직위 (구체적 업무는 description에)
